...
class Podcast:
    def __init__(self, model: ChatterboxTurboTTS, voice_map: Dict[str, str], root_dir: str):
        self.model = model
    ...
    def create(self, args: ConversationInput) -> torch.Tensor:
        ...
        for i, segment in enumerate(args["segments"]):
            ...
            logger.info(f"[{progress:.1f}%] Processing segment {i+1}/{total_segments} ({voice}): {text[:50]}...")
            
            try:
                prompt_path = self._get_audio_prompt_path(voice)
                wav = self.model.generate(
                    text, audio_prompt_path=prompt_path
                )
                segments.append(wav)
